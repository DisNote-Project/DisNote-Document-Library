...
const assert = {
  equal(actual: unknown, expected: unknown, message?: string): void {
    if (actual !== expected)
      throw new Error(
        message ?? LIBRARY_MESSAGES.expectedValue(expected, actual)
      );
  },
  deepEqual(actual: unknown, expected: unknown, message?: string): void {
    if (JSON.stringify(actual) !== JSON.stringify(expected)) {
      throw new Error(
        message ?? LIBRARY_MESSAGES.expectedJson(expected, actual)
      );
    }
  },
};
...
export function runDocumentRepositoryContract(
  ...
  register("getById returns a seeded document", async () => {
    ...
    const id = await h.seed({
      slug: "s1",
      locale: "en",
      title: "T",
      document: h.makeDocument("d", "s1", "hi"),
    });
  ...
  register("saveDraft bumps the revision", async () => {
    ...
    const id = await h.seed({
      ...
      locale: "en",
      title: "T",
      document: h.makeDocument("d", "s2", "v1"),
    });
  ...
  register("saveDraft rejects a stale expectedRevision", async () => {
    ...
    const id = await h.seed({
      ...
      locale: "en",
      title: "T",
      document: h.makeDocument("d", "s3", "v1"),
    });
    await h.draftWriter.saveDraft({
      documentId: id,
      expectedRevision: 1,
      document: h.makeDocument(id, "s3", "v2"),
      idempotencyKey: "k1",
      actor: "u1",
    });
    const stale = await h.draftWriter.saveDraft({
      documentId: id,
      expectedRevision: 1,
      document: h.makeDocument(id, "s3", "v3"),
      idempotencyKey: "k2",
      actor: "u1",
    });
  ...
  register("idempotency key does not duplicate a revision", async () => {
    ...
    const id = await h.seed({
      ...
      locale: "en",
      title: "T",
      document: h.makeDocument("d", "s4", "v1"),
    });
    const a = await h.draftWriter.saveDraft({
      documentId: id,
      expectedRevision: 1,
      document: h.makeDocument(id, "s4", "v2"),
      idempotencyKey: "same",
      actor: "u1",
    });
    const b = await h.draftWriter.saveDraft({
      documentId: id,
      expectedRevision: 1,
      document: h.makeDocument(id, "s4", "v2"),
      idempotencyKey: "same",
  ...
  register("idempotency keys are scoped to one document", async () => {
    const h = await makeHarness();
    const firstId = await h.seed({
      slug: "idem-a",
      locale: "en",
      title: "A",
      document: h.makeDocument("idem-a", "idem-a", "v1"),
    });
    const secondId = await h.seed({
      slug: "idem-b",
      locale: "en",
      title: "B",
      document: h.makeDocument("idem-b", "idem-b", "v1"),
    });
  ...
  register(
    "published revision is immutable while a new draft is written",
    async () => {
      const h = await makeHarness();
      const id = await h.seed({
        slug: "policy",
        locale: "en",
        title: "P",
        document: h.makeDocument("d", "policy", "live"),
      });
      await h.publisher.publish({ documentId: id, revision: 1, actor: "u1" });
      await h.draftWriter.saveDraft({
        documentId: id,
        expectedRevision: 1,
        document: h.makeDocument(id, "policy", "edited"),
        idempotencyKey: "k",
        actor: "u1",
      });
      const published = await h.reader.getPublishedBySlug({
        slug: "policy",
        locale: "en",
      });
  ...
  register("archived document is not served publicly", async () => {
    ...
    const id = await h.seed({
      ...
      locale: "en",
      title: "O",
      document: h.makeDocument("d", "old", "x"),
    });
    await h.publisher.publish({ documentId: id, revision: 1, actor: "u1" });
    await h.publisher.archive({ documentId: id, actor: "u1" });
    assert.equal(
      await h.reader.getPublishedBySlug({ slug: "old", locale: "en" }),
      null
    );
  });
}
