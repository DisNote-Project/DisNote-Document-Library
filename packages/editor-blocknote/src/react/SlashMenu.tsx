...
import { defaultSlashCommands, filterSlashCommands, type SlashCommand } from "../slash-menu/commands.js";
 ...
 */
export function SlashMenu({ i18n, query, commands = defaultSlashCommands, onSelect }: SlashMenuProps): ReactElement {
  const results = useMemo(() => filterSlashCommands(query, commands), [query, commands]);
      ...
      >
        {results.map((cmd, i) => (
          <li
            ...
            style={{ padding: "6px 8px", borderRadius: 6, background: i === clamped ? "#f0f0f0" : "transparent", cursor: "pointer" }}
          >
            {i18n.t(cmd.titleKey)}
          </li>
        ))}
        {results.length === 0 ? <li style={{ padding: "6px 8px", color: "#5c6666" }}>No matches</li> : null}
      </ul>
    </div>
  );
}
